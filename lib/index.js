...
class EthEvents {
  ...
  async getRawLogs(filter) {
    console.log(`Searching: ${filter.get('fromBlock')} .. ${filter.get('toBlock')}`)
    const rawLogs = await this.eth.getLogs(filter.toJS())
  ...
  async batchGetLogs(filter, finalBlock, logs = []) {
    try {
      console.log('')
      console.log('# subtotal accLogs:', logs.length)

      // CASE: to_block is greater than or equal to the final_block. get logs
      if (filter.get('toBlock') >= finalBlock) {
        // do not exceed the finalBlock
        const lastFilter = filter.set('toBlock', finalBlock)

        const lastRawLogs = await this.getRawLogs(lastFilter)
        // concat the accumulation of logs
        const finalAccLogs = logs.concat(lastRawLogs)

        console.log('')
        console.log('# final accLogs:', finalAccLogs.length)
        console.log('')

        // decode logs, return normalized logs
        const decodedLogs = this.decoder(finalAccLogs)
        return this.normalizeLogs(finalAccLogs, decodedLogs)
      }
      ...
      const newFilter = filter
        .set('fromBlock', filter.get('toBlock') + 1)
        .set('toBlock', filter.get('toBlock') + this.blockRangeThreshold)
