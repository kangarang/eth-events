  ...
  async batchGetLogs(filter, finalBlock, logs = []) {
    try {
      ...
      if (filter.toBlock >= finalBlock) {
        ...
        console.log(`Searching: ${filter.fromBlock} .. ${filter.toBlock}`)
        ...
        console.log('')
        console.log('# final accLogs:', accLogs.length)
        console.log('')
      ...
      if (filter.toBlock - filter.fromBlock === this.blockRangeThreshold) {
        // the first time this function is executed, the fromBlock should not be edited
        newFilter.fromBlock = filter.fromBlock
      }
      console.log(`Searching: ${newFilter.fromBlock} .. ${newFilter.toBlock}`)
